...
export class ScreenplayParser implements IScreenplayParser {
  ...
  private getScriptTitle(pageLines: PDFLine[]): string {
    ...
    let title = "";
    const titleLines = pageLines.filter(
      (line) => !!line.str.replace(/ /g, "") && line.height === maxHeight
    );
    ...
    if (titleLines.length === pageLines.length) {
      return title;
    }
  ...
  private getWrittenBy(pageLines: PDFLine[]): string {
    if (!!this.metadata.writtenBy) return this.metadata.writtenBy;
    ...
    try {
      ...
      if (!this.metadata.titlePage) {
        if (this.parser.isTitlePage(pageLines)) {
          this.metadata.titlePage = this.readPage + 1; // the readPage is always inferior to the current iteration, so +1
          this.metadata.title = this.getScriptTitle(pageLines);
          this.metadata.writtenBy = this.getWrittenBy(pageLines);
          this.metadata.email = this.getEmail(pageLines);
          this.metadata.phone = this.getPhone(pageLines);
          this.readPage++;
          return;
        }
      }
  ...
  private parseEnrichedPage(pageLines: PDFLine[]) {
    ...
    for (let i = 0; i < pageLines.length; i++) {
      ...
      if (this.parser.isCharacter(line)) {
        ...
        this.isPrevLineDialog = false;
        this.sceneIndex++;
        this.scenesPresence.push({ scene: line.str, characters: [] });
